...
contract FeesFacet is IFeesFacet {
    ...
    function setIntegratorInfo(
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            if (
                feeInfo.dzapTokenShare > LibFees._BPS_DENOMINATOR ||
                feeInfo.dzapFixedNativeShare > LibFees._BPS_DENOMINATOR
            ) {
                revert ShareTooHigh();
            }

            if (
                feeInfo.tokenFee > LibFees._BPS_DENOMINATOR ||
                feeInfo.tokenFee > fs.maxTokenFee
            ) {
                revert FeeTooHigh();
            }

            if (feeInfo.fixedNativeFeeAmount > fs.maxFixedNativeFeeAmount) {
                revert FeeTooHigh();
            }

            if (!fs.integratorInfo[_integrator].status)
                fs.integratorInfo[_integrator].status = true;
    ...
    function calcFixedNativeFees(
        ...
        (fixedNativeFeeAmount, dzapShare) = LibFees.calcFixedNativeFees(
            _feeType,
            _integrator
        );
    }
