...
library LibFees {
     ...
     */
    function accrueFixedNativeFees(bytes32 _transactionId, address _integrator, FeeType _feeType) internal {
        ...
        if (fixedNativeFeeAmount > 0) {
            dzapShare = (fixedNativeFeeAmount * feeInfo.dzapFixedNativeShare) / _BPS_DENOMINATOR;

            if (fixedNativeFeeAmount - dzapShare > 0) LibAsset.transferNativeToken(_integrator, fixedNativeFeeAmount - dzapShare);
        }

        if (dzapShare > 0) LibAsset.transferNativeToken(fs.protocolFeeVault, dzapShare);

        emit FixedNativeFee(_transactionId, _integrator, _feeType, dzapShare, fixedNativeFeeAmount - dzapShare);
    }

    function accrueTokenFees(bytes32 _transactionId, address _integrator, FeeType _feeType, address _token, uint256 _integratorFee, uint256 _dZapFee) internal {
        FeesStorage storage fs = feesStorage();

        if (_integratorFee > 0) LibAsset.transferToken(_token, _integrator, _integratorFee);
        if (_dZapFee > 0) LibAsset.transferToken(_token, fs.protocolFeeVault, _dZapFee);

        emit TokenFee(_transactionId, _integrator, _feeType, _token, _dZapFee, _integratorFee);
    }
