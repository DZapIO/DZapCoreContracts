...
contract MockBridge {
    ...
    address public mockBridgeRelayer;
    ...
    function updateNativeFeeAmount(uint256 _nativeFeeAmount) external {
        nativeFeeAmount = _nativeFeeAmount;
    }

    function bridge(address recipient_, address token_, uint256 amount_, bool testRevert_) external payable {
        if (testRevert_) {
    ...
    function bridge2(address token_, uint256 amount_, bool testRevert_) external payable {
        if (testRevert_) {
            revert BridgeCallFailedFromRouter();
        }
        ...
        if (isNative(token_)) {
            (bool success, ) = mockBridgeRelayer.call{ value: amountWithoutFee }("");
            if (!success) revert NativeTransferFailed();
        } else {
            IERC20(token_).safeTransferFrom(msg.sender, address(this), amount_);
            IERC20(token_).safeTransfer(mockBridgeRelayer, amountWithoutFee);
        }
    }
    ...
    function bridgeAndSwap(
        ...
        (bool success, bytes memory res) = callTo_.call{ value: nativeValue }(callData_);
