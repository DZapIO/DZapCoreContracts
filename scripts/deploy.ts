...
async function init() {
  ...
  // add fee data
  {
    ...
    const tx = await feesFacet
      .connect(owner)
      .setIntegratorInfo(integratorAddress, [FeeType.SWAP], [feeInfo])
  ...
  /* 
  {
    ...
    let selectors = getSelectorsUsingFunSig([
      // kyber
...
async function deployFacetsToReplace(facetNames: string[]) {
  const cutData: DiamondCut[] = []
  console.log('')
  console.log('Deploying NewFacets:', cutData)
