...
describe('BatchSwapFacet.test.ts', async () => {
  beforeEach(async () => {
  ...
  describe('1) batchSwap', async () => {
    it('1.1 Should allow user to swap token (Erc20 -> Erc20)', async () => {
      ...
      await expect(
        batchSwapFacet
          .connect(user)
          .batchSwap(transactionId, integratorAddress, recipient, swapData, [
            true,
            false,
          ])
      ).emit(batchSwapFacet, EVENTS.MultiSwapped)
      ...
      const eventFilter = batchSwapFacet.filters.MultiSwapped()
      const data = await batchSwapFacet.queryFilter(eventFilter)
      ...
      expect(args.swapInfo[0].returnToAmount).equal(minAmount)
      ...
      expect(args.swapInfo[1].returnToAmount).equal(minAmount)

      expect(userBalanceAfterB).equal(userBalanceBeforeB.sub(amounts[0].mul(2)))
      expect(recipientBalanceAfterA).equal(
        recipientBalanceBeforeA.add(minAmount.mul(2))
      )

      // ----------------------------------------------------------------
    })

    it('1.2 Should allow user to swap multiple tokens(erc20 -> native, erc20 -> erc20)', async () => {
      const rate = await mockExchange.rate()
      ...
      await tokenA
        .connect(user)
        .approve(batchSwapFacet.address, parseUnits('100', TOKEN_A_DECIMAL))
    ...
    it('1.3 Should allow user to swap multiple tokens(erc20 -> erc20, native -> erc20)', async () => {
      const rate = await mockExchange.rate()
    ...
    it('1.4 Should allow user to swap multiple tokens and return leftover and extra native(erc20 -> erc20, native -> erc20)', async () => {
      const rate = await mockExchange.rate()
    ...
    it('1.5 Should allow user to swap tokens, using permit approve (Erc20 -> Erc20, native -> erc20)', async () => {
      const rate = await mockExchange.rate()
      ...
      await tokenA.mint(user.address, parseUnits('100', TOKEN_A_DECIMAL))
      await tokenA.connect(user).approve(batchSwapFacet.address, ZERO)
      ...
      const eventFilter = batchSwapFacet.filters.MultiSwapped()
      const eventData = await batchSwapFacet.queryFilter(eventFilter)
    ...
    it('1.7 Should allow user to swap tokens, using permit2 approve (Erc20 -> Erc20, Erc20 -> Erc20)', async () => {
      const rate = await mockExchange.rate()
    ...
    it('1.8 Should revert if recipient is zero address', async () => {
      // ----------------------------------------------------------------
      // native to tokenA
      const user = signers[12]
      const transactionId = ethers.utils.formatBytes32String('dummyId')
      ...
      await expect(
        batchSwapFacet.connect(user).batchSwap(
          transactionId,
          integratorAddress,
          ZERO_ADDRESS,
          [
      ...
      ).revertedWithCustomError(batchSwapFacet, ERRORS.ZeroAddress)
    })
    ...
    it('1.9 Should revert if integrator is not allowed', async () => {
      ...
      ).revertedWithCustomError(batchSwapFacet, ERRORS.IntegratorNotAllowed)
    })
    ...
    it('1.10 Should revert if from amount is zero', async () => {
      ...
      ).revertedWithCustomError(batchSwapFacet, ERRORS.NoSwapFromZeroBalance)
    })
    ...
    it('1.11 Should revert if callTo(dex) is not approved', async () => {
      ...
      ).revertedWithCustomError(batchSwapFacet, ERRORS.ContractCallNotAllowed)
    })
    ...
    it('1.13 Should revert if swap call fails', async () => {
      ...
      await expect(
        ...
        .revertedWithCustomError(batchSwapFacet, ERRORS.SwapCallFailed)
    ...
    it('1.14 Should revert if slippage is too high', async () => {
      ...
      await expect(
        ...
        .revertedWithCustomError(batchSwapFacet, ERRORS.SlippageTooLow)
        .withArgs(swapData[0].minToAmount, minAmount)
    })

    it('1.14 Should revert if wrong transfer type is send', async () => {
      const rate = await mockExchange.rate()
      ...
      await expect(
        ...
        .revertedWithCustomError(batchSwapFacet, ERRORS.SlippageTooLow)
        .withArgs(swapData[0].minToAmount, 0)
    })
  })
})
